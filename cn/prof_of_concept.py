...
for i in range(len(old)):
    print(len(old[i]), len(cha[i]),len(new[i]))
...
#output.shape = (b, seq_lenght, vocab_size)
#sampled_idx.shape = (b, seq_lenght, 1)
output, sampled_idx = model(input_batch,change_batch,target_batch)

print(f'output: {output.sahpe}')
print(f'sampled_idx: {sampled_idx.sahpe}')
