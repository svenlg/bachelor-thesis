...
def train(rank, args):
    ...
    for epoch in range(1,args.epochs+1):
        ...
        if cur_low_val_eval > val_loss and epoch > 3:
            ...
            best_round = epoch
            save_path = f'/scratch/sgutjahr/log/{args.model_name}_COPY_best.pt'
            torch.save({'epoch': epoch,
                        'model_state_dict': encoder_decoder.module.state_dict(),
                        'loss': cur_low_val_eval}, save_path)
            
        if epoch % 2 == 0:
            np.save(f'/scratch/sgutjahr/log/{args.model_name}_COPY_train_epoch_{epoch}.npy', np.array(loss_train))
            np.save(f'/scratch/sgutjahr/log/{args.model_name}_COPY_val_epoch_{epoch}.npy', np.array(loss_val))


    print(f'Lowest validation loss: {cur_low_val_eval:.4f} in Round {best_round}')
    np.save(f'/scratch/sgutjahr/log/{args.model_name}_COPY_train.npy', np.array(loss_train))
    np.save(f'/scratch/sgutjahr/log/{args.model_name}_COPY_val.npy', np.array(loss_val))
