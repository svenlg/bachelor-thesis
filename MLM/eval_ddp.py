...
# Evaluate Model
def evaluate(model, val_loader, gpu, mask):
    ...
    with torch.no_grad():
        ...
        for batch in val_loader:
            ...
            input_ids_gpu = batch['input_ids'].cuda(gpu)
            attention_mask = batch['attention_mask'].cuda(gpu)
            labels_cpu = batch['labels']
            labels_gpu = batch['labels'].cuda(gpu)
